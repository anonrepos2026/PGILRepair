...
void Coordinator::registerOnlineEC(unsigned int clientIp, string filename, string ecid, int filesizeMB) {
  ...
  std::cout << "UL: ";
  for(int i=0;i<ecn;i++) std::cout << ecpolicy->getULconst(i) << " ";
  std::cout << "\nDL: ";
  for(int i=0;i<ecn;i++) std::cout << ecpolicy->getDLconst(i) << " ";
  std::cout << "\n";
  ...
  if (ecid == "rs_prop" || ecid == "rs_prop_rand"){
    ssentry = new SSEntry(filename, -1, filesizeMB, ecid, objnames, ips);
  }else if(ecid == "rs_pivot"){
    ssentry = new SSEntry(filename, -2, filesizeMB, ecid, objnames, ips);
  }else{
    ssentry = new SSEntry(filename, 0, filesizeMB, ecid, objnames, ips);
  }
...
unsigned int Coordinator::chooseFromCandidates(vector<unsigned int> candidates, string policy, string type) {
    // Helper Selection part: Should be fixed
  if (policy == "random") {
...
void Coordinator::repairReqFromSS(CoorCommand* coorCmd) {
  ...
  SSEntry* ssentry = _stripeStore->getEntryFromObj(objname);
  int redundancy = ssentry->getType();
  // redundancy ==0(online) -1(prop offline) else(offline)
  if (redundancy == 0) {
    return recoveryOnline(objname);
  } else if(redundancy == -1){
    return recoveryPropOnline(objname);
  } else if (redundancy == -2){
    return recoveryPivotOnline(objname);
  } else {
...
void Coordinator::recoveryOnline(string lostobj) {
  ...
  // after we create commands, we send these commands to corresponding Agent
  ...
  // relocate  Group constraints are unnecessary... we simplify the following codes
  ...
  //Seems indepedent operation from the above(?)
  vector<int> toposeq = ecdag->toposort();
...
void Coordinator::recoveryPropOnline(string lostobj) {
  // obtain needed information
  // 25/05/10 Modified from carisis(needs to be revised further)
  // we need ecdag, toposort and parseForOEC, which requires cid2ip, stripename, n,k,w,pktnum,objlist
  // we also need to create persist command to persist repaired block
  // after we create commands, we send these commands to corresponding Agent
  ...
  int eck = ecpolicy->getK();
  int ecw = ecpolicy->getW();
  int ecl = ecpolicy->getl();
  bool locality = ecpolicy->getLocality();
  int opt = 0;//ecpolicy->getOpt();
  
  
  vector<int> availcidx;
  vector<int> toreccidx;    //Assumption: Only works for single node failure

  //Seems not necessary now..
  vector<int> ulavcidx; //Storing the node index of uplink BW constraints
  //vector<int> dlavcidx; //Storing the the node index of downlink BW constraints
  int dlfailidx;     //  to sotre the BW of downlink BW for the failed nodes
  
  vector<int> ulavcBW; //Storing the BW of uplink BW constraints
  vector<int> dlavcBW; //Storing the the BW of downlink BW constraints
  int dlfailBW;        //sotring the BW of downlink BW for the failed nodes

  for (int i=0; i<ecn; i++) {
    if ( lostidx.end() == find(lostidx.begin(), lostidx.end(), i)) {
      for (int j=0; j<ecw; j++) availcidx.push_back(i*ecw+j);
      ulavcBW.push_back(ecpolicy->getULconst(i));
      dlavcBW.push_back(ecpolicy->getDLconst(i));
    } else {
      for (int j=0; j<ecw; j++) toreccidx.push_back(i*ecw+j);
      dlfailidx = i;
      dlfailBW = ecpolicy->getDLconst(i);
    }
  }
  //Sorted version
  vector<int> ulsortidx(availcidx.size()); //Storing the 'sorted' uplink BW constraints
  vector<int> dlsortidx(eck); //Storing the 'sorted' downlink BW constraints

  vector<int> ulsortBW(availcidx.size()); 
  vector<int> dlsortBW(eck); 

  vector<vector<vector<int>>> pipeidx(ecl); //Intialize only for 1st dim(num. of repair path)
  //Result of each pipeidx in each repair path index [repair path][depth][PG(idx 0)+Leaf(>1)]

  //Initialization of the 'sorted' vector value

  vector<int> targetBW(ecl); //Targeted BW for UL-based LBFS algorithm(size)
  vector<int> realBW(ecl); //Realized BW by DL ECDAG graph, ecl

  for(int l = 0 ; l < ecl ; l++)
  {
      ulavcidx = availcidx; //Initialization for each repair path
      ulsortBW = ulavcBW;   //Initialization for sorted BW


    //Step 1: construct good-bandwdith helper selection according to the LBFS algorithm
    vector<int> ulselcidx(ulavcidx.size());  //helper selection
    vector<int> ultmpcidx(ulavcidx.size());
    for (int i = 0; i < ulavcidx.size(); i++)  {ulselcidx[i] = ulavcidx[i]; ultmpcidx[i] = i;}
    //cout << "\n**********ul***********\n";
    //for(int i=0;i<ulsortBW.size();i++) { cout << "\n" << ulselcidx[i] << " / " << ulsortBW[i] << endl; }

    sort(ultmpcidx.begin(), ultmpcidx.end(),
    [&ulsortBW](int a, int b)->bool { return ulsortBW[a] > ulsortBW[b]; });

    //25.07.01
    vector<int> ultmpsortBW = ulsortBW;
    vector<int> ultmpselcidx = ulselcidx;

    for(int i=0;i<ulavcidx.size();++i) { ulsortBW[i] = ultmpsortBW[ultmpcidx[i]]; ulselcidx[i] = ultmpselcidx[ultmpcidx[i]]; }

    //for(int i=0;i<ulsortBW.size();i++) { cout << "\n after sort" << ulselcidx[i] << " / " << ulsortBW[i] << endl; }

    // LBFS algorithmic sorting from the largest to smaller one, which changes the ulselcidx only,
    //,which can select the largest k helpers according to the uplink
    // NOTE: Real node idx can be represented by DL availcidx(ulselcidx[i]))
    targetBW[l] = ulsortBW[ulselcidx[eck-1]]; //Targeted BW for LBFS algorithm
    realBW[l] =  targetBW[l];   // Realized BW which can be smaller than the targeted BW(by ECDAG construction rule)

    //Step 2: construct ECDAG according to DL constraints
    vector<int> dlselcidx(eck);   //index on [k]
    vector<int> dltmpcidx(eck);
    for (int i = 0; i < eck; i++)  {dlselcidx[i] = ulselcidx[i]; dlsortBW[i] = dlavcBW[dlselcidx[i]]; dltmpcidx[i] = i;}
    //cout << "\n***********dl**************\n";
    //for (int i=0;i<eck;i++){ cout << "\n" << dlselcidx[i] << " / " << dlsortBW[i] << endl; }

    sort(dltmpcidx.begin(), dltmpcidx.end(),
    [&dlsortBW](int a, int b)->bool { return dlsortBW[a] > dlsortBW[b]; });

    vector<int> dltmpsortBW = dlsortBW;
    vector<int> dltmpselcidx = dlselcidx;

    for(int i=0; i < eck; i++){ dlsortBW[i] = dltmpsortBW[dltmpcidx[i]]; dlselcidx[i] = dltmpselcidx[dltmpcidx[i]]; }

    //for(int i=0;i<eck;i++){ cout << "\n*" << dlselcidx[i] << " / " << dlsortBW[i] << endl; }
    //Find the number of possible leafs
    int numECPipe = 0, numECLeaf = 0; //Count for pipelining and leaf
    //Pipelined node: Nodes w. UL and DL
    //Leaf node: Nodes only w. UL(no DL)

    //unit Pipelining graph and leafs addition
    int numLeaf;
    if(targetBW[l] != 0){   //initialization value for numLeaf(d=0)
      numLeaf = min(dlfailBW/targetBW[l]-1,eck-numECPipe-numECLeaf-1);
    }else{
      numLeaf = 0;
    }
   ...
   // set<int> usedNodes;
    ...
    while( numECPipe + numECLeaf < eck)
    {
      //unit Pipelining graph addition from the front(least idx)

      //Check the DL constraints w. ( availDL for the last depth < targetDL)
      
      pipeidx[l].push_back({dlselcidx[numECPipe]});
      
      if(numECPipe + numECLeaf + 1 == eck) break;

      for(int j = 0 ; j < numLeaf ; j++)
      {
        if( numECPipe + numECLeaf < eck )
        {
          int leafIdx = eck-1-numECLeaf;
   //       if(usedNodes.find(dlselcidx[leafIdx]) == usedNodes.end()){
            //Single failed node downlink not considered: TOBEREIVSED
            pipeidx[l][numECPipe].push_back(dlselcidx[leafIdx]);
            //usedNodes.insert(dlselcidx[leafIdx]);
            numECLeaf++;
  //      }
        }
        else
      ...
      if(targetBW[l] != 0){
        realBW[l] = min(realBW[l],dlsortBW[numECPipe]);
        if(realBW[l] == 0) break;
        
        numLeaf = min(dlsortBW[numECPipe]/realBW[l]-1,eck-numECPipe-numECLeaf-2);
        numECPipe++;
      }
      else{
        numLeaf = 0;
    ...
    //Available BW reduction for UL(k helpers), DL(Pipelined case)
    for(int j = 0 ; j < eck ; j++)
    {
      ulavcBW[ulselcidx[j]] = ulavcBW[ulselcidx[j]]-realBW[l]; 
    }
    for(int j = 0 ; j < numECPipe; j++ )
    {
      int tmpECPipe = pipeidx[l][j].size();   //tep num.leaf.size;
      dlavcBW[dlselcidx[j]] = dlavcBW[dlselcidx[j]]-tmpECPipe*realBW[l];
    }
    //Pipelining part: additional DL reduction by num of leafs

}
  cout << "***********pipeidx**********" << endl;
  for(int d =0;d<pipeidx[0].size();d++){
    cout << "depth: " << d << endl;
    for(int j=0;j<pipeidx[0][d].size();j++){
      cout << pipeidx[0][d][j] << "\n";
    }
  }
  //Should Add the conditional select
  ECDAG** ecdag_path = new ECDAG*[ecl];
  for(int l = 0; l < ecl; l++ )
  {
     ecdag_path[l] = ec->PipeDecode(pipeidx[l], toreccidx);
     ecdag_path[l]->reconstruct(0);

  }

  //Fix opt as 0
  //25.05.13 Should implement after
  // prepare sid2ip, for cip2ip
  // prepare stripeips for client info
  unordered_map<int, unsigned int> sid2ip;
  unordered_map<int, pair<string, unsigned int>> objlist;
  ...
  for (int i=0; i<ecn; i++) {
    if (integrity[i] == 1) { 
      ...
      vector<int> colocWith;
      //if (idx2group.find(i) != idx2group.end()) colocWith = idx2group[i]; //Group constraints would not be necessary
      vector<unsigned int> candidates = getCandidates(placedIps, placedIdx, colocWith);
      // we need to remove remaining ips in candidates
      for (int j=i+1; j<ecn; j++) {
        if (integrity[j] == 1) {
      ...
      objlist[i].second = curip;
      stripeips[i] = curip;
      // we need to update the location in SSEntry(Maybe, failure option can be updated)
      string objname = objlist[i].first;
      SSEntry* curssentry = _stripeStore->getEntryFromObj(objname);
      curssentry->updateObjLoc(objname, curip);
    }
  }
    //Should modify according to our pre-defined pipelining architecture
      unordered_map<int, AGCommand*> agCmds;
      vector<AGCommand*> persistCmds;
    //define early
for(int l = 0; l < ecl; l++ )
{
  vector<int> toposeq = ecdag_path[l]->toposort();

  // prepare cid2ip, for parseForOEC
  unordered_map<int, unsigned int> cid2ip;
  for (int i=0; i<toposeq.size(); i++) {
    int cidx = toposeq[i];
    ECNode* node = ecdag_path[l]->getNode(cidx);
    vector<unsigned int> candidates = node->candidateIps(sid2ip, cid2ip, _conf->_agentsIPs, ecn, eck, ecw, locality, lostidx[0]);
    // choose from candidates
    unsigned int curip = chooseFromCandidates(candidates, _conf->_repair_policy, "repair");
    cid2ip.insert(make_pair(cidx, curip));
  }
  ...
  // 6. parse for oec
  //vector<AGCommand*> agCmds = ecdag->parseForOEC(cid2ip, stripename, ecn, eck, ecw, pktnum, objlist);
   auto m=ecdag_path[l]->parseForOEC(cid2ip, stripename, ecn, eck, ecw, pktnum, objlist);
   agCmds.insert( m.begin(), m.end());
   
  
  //agCmd: Combined command for load & cache and fetch & compute & cache

  // 7. add persist cmd
   auto m2 =ecdag_path[l]->persist(cid2ip, stripename, ecn, eck, ecw, pktnum, objlist);
   persistCmds.insert(persistCmds.end(), m2.begin(), m2.end());
  
  //PersistCmd: Persist(: It either writes a block to HDFS via the HDFS client, or returns the block to an OECClient)
  // 8. send commands to cmddistributor
}
...
void Coordinator::recoveryPivotOnline(string lostobj) {
  // obtain needed information
  // 25/12/13 Modified from jeonjiho817
  // we need ecdag, toposort and parseForOEC, which requires cid2ip, stripename, n,k,w,pktnum,objlist
  // we also need to create persist command to persist repaired block
  // after we create commands, we send these commands to corresponding Agent
  ...
  for(int l = 0 ; l < ecl ; l++)
  {
    //ulavcidx = availcidx; => UL을 기준으로 정렬할 필요 없음
    //ulsortBW = ulavcBW;

    //Step 1: Helper selection & Tree construction
    vector<int> helpers;  //Storing the helper node index
    for(int idx : availcidx){
      helpers.push_back(idx/ecw);
    }

    vector<pair<int,int>> theo_list; //Storing the helper node and its theoretical BW
    for(int h : helpers){
      int uplink = ulavcBW[h];
      int downlink = dlavcBW[h];
      int theo = min(uplink, downlink);
      theo_list.push_back({h, theo});
    }

    sort(theo_list.begin(), theo_list.end(),
      [&](pair<int,int> &a, pair<int,int> &b){ 
        ...
        if(dlavcBW[node_a] != dlavcBW[node_b]){
          return dlavcBW[node_a] > dlavcBW[node_b];
        }
        return ulavcBW[node_a] > ulavcBW[node_b];
      });
    ...
    struct cmp {
      bool operator()(const Pivot &a, const Pivot &b) {
        return a.prac < b.prac;
      }
    };

    priority_queue<Pivot, vector<Pivot>, cmp> Q;
    unordered_map<int, vector<int>> tree;
    unordered_map<int, int> parentMap;
    unordered_map<int, int> pracMap;
    unordered_map<int, int> degreeMap;
    unordered_set<int> selected;
    //vector<int> inserted_list;

    tree[dlfailidx] = {};
    parentMap[dlfailidx] = -1;
    pracMap[dlfailidx] = dlfailBW;
    degreeMap[dlfailidx] = 0;

    Q.push({dlfailidx, dlfailBW, 0});


    for(int i=0;i<eck;i++){
        Pivot tmp = Q.top();
        int parent = tmp.id;
        int parent_prac = tmp.prac;
        int parent_degree = tmp.degree;
        Q.pop();

        int child = theo_list[i].first;

        tree[parent].push_back(child);
        parentMap[child] = parent;
        selected.insert(child);
        //inserted_list.push_back(child);
        degreeMap[parent]++;

        pracMap[child] = min(ulavcBW[child], dlavcBW[child]);
        pracMap[parent] = min(ulavcBW[parent], dlavcBW[parent] / (degreeMap[parent] + 1));

        Q.push({parent, pracMap[parent], degreeMap[parent]});
        Q.push({child, pracMap[child], 0});
    }
    //Step 2: Replacement of leaf nodes
    vector<int> leaf_nodes;
    for (auto &p : tree) {
        int node = p.first;
        if (tree[node].empty()) leaf_nodes.push_back(node);
    }

    vector<int> candidate_pool = leaf_nodes;

    for (auto &h_pair : theo_list) {
        int h = h_pair.first;
        if (!selected.count(h)) candidate_pool.push_back(h);
    }

    sort(candidate_pool.begin(), candidate_pool.end(), [&](int a, int b){
        return ulavcBW[a] > ulavcBW[b];
    });

    vector<int> new_leaf(candidate_pool.begin(), candidate_pool.begin() + leaf_nodes.size());
    unordered_set<int> new_leaf_set(new_leaf.begin(), new_leaf.end());
    vector<int> Lreplaced;

    for (int lf : leaf_nodes) {
        if (!new_leaf_set.count(lf)) {
            Lreplaced.push_back(lf);
        }
    }

    vector<int> Lstrong;
    for (int nf : new_leaf) {
        if (!count(leaf_nodes.begin(), leaf_nodes.end(), nf)) {
            Lstrong.push_back(nf);
        }
    }

    for (int i = 0; i < Lreplaced.size(); i++) {
        int weak = Lreplaced[i];
        int strong = Lstrong[i];
        int parent = parentMap[weak];
        auto &ch = tree[parent];

        ch.erase(remove(ch.begin(), ch.end(), weak), ch.end());
        selected.erase(weak);

        tree[parent].push_back(strong);
        parentMap[strong] = parent;
        selected.insert(strong);
    }
    ...
    while (!q.empty()) {
        ...
        if(row.size() < 2) continue; //leaf node skip
        pipeidx[l].push_back(row);
    }
  ...
  for(int l = 0; l < ecl; l++ )
  {
     ecdag_path[l] = ec->PivotDecode(pipeidx[l], toreccidx);
     ecdag_path[l]->reconstruct(0);
