...
vector<unsigned int> ECNode::candidateIps(unordered_map<int, unsigned int> sid2ip,
  ...
  if (locality) {
    ...
    if (toret.size() == 0) {
      // choose randomly(Existing implementation)
      srand((unsigned)time(0));
      int randomidx = rand() % childIps.size();
      toret.push_back(childIps[randomidx]);

      //For the proposed case, only failed node(newcomer) can be applied
      //Therefore, we just select the last one(by the failure sceanrio)

      //toret.push_back(childIps[childIps.size()-1]);
    }
    return toret;
  }
}
...
vector<unsigned int> ECNode::candidateIps(unordered_map<int, unsigned int> sid2ip,
  ...
  // 1. current node is preassigned a location
  // Original if (sid2ip.find(sid) != sid2ip.end() && sid != lostid) {
  if (sid2ip.find(sid) != sid2ip.end()) {
    toret.push_back(sid2ip[sid]);
    return toret;
  }
